 ...
 */
MKL_OneButtonTiny::MKL_OneButtonTiny(const int pin, const boolean activeLow, const bool pullupActive) {
  _pin = pin;
...
void MKL_OneButtonTiny::setDebounceMs(const unsigned int ms) {
  _debounce_ms = ms;
}  // setDebounceMs


// explicitly set the number of millisec that have to pass by before a click is detected.
void MKL_OneButtonTiny::setClickMs(const unsigned int ms) {
  _click_ms = ms;
}  // setClickMs


// explicitly set the number of millisec that have to pass by before a long button press is detected.
void MKL_OneButtonTiny::setPressMs(const unsigned int ms) {
  _press_ms = ms;
}  // setPressMs


// save function for click event
void MKL_OneButtonTiny::attachClick(callbackFunction newFunction) {
  _clickFunc = newFunction;
}  // attachClick


// save function for doubleClick event
void MKL_OneButtonTiny::attachDoubleClick(callbackFunction newFunction) {
  _doubleClickFunc = newFunction;
}  // attachDoubleClick


// save function for longPressStart event
void MKL_OneButtonTiny::attachLongPressStart(callbackFunction newFunction) {
  _longPressStartFunc = newFunction;
}  // attachLongPressStart


void MKL_OneButtonTiny::reset(void) {
  _state = MKL_OneButtonTiny::OCS_INIT;
  _nClicks = 0;
  _startTime = 0;
}
 ...
 */
int MKL_OneButtonTiny::debounce(const int value) {
 ...
 */
void MKL_OneButtonTiny::tick(void) {
  if (_pin >= 0) {
...
void MKL_OneButtonTiny::tick(bool activeLevel) {
 ...
 */
void MKL_OneButtonTiny::_newState(stateMachine_t nextState) {
  _state = nextState;
}  // _newState()
 ...
 */
void MKL_OneButtonTiny::_fsm(bool activeLevel) {
  unsigned long waitTime = (now - _startTime);

  // Implementation of the state machine
  switch (_state) {
    case MKL_OneButtonTiny::OCS_INIT:
      // waiting for level to become active.
      if (activeLevel) {
        _newState(MKL_OneButtonTiny::OCS_DOWN);
        _startTime = now;  // remember starting time
        _nClicks = 0;
      }  // if
      break;

    case MKL_OneButtonTiny::OCS_DOWN:
      // waiting for level to become inactive.

      if (!activeLevel) {
        _newState(MKL_OneButtonTiny::OCS_UP);
        _startTime = now;  // remember starting time

      } else if ((activeLevel) && (waitTime > _press_ms)) {
        if (_longPressStartFunc) _longPressStartFunc();
        _newState(MKL_OneButtonTiny::OCS_PRESS);
      }  // if
      break;

    case MKL_OneButtonTiny::OCS_UP:
      // level is inactive

      // count as a short button down
      _nClicks++;
      _newState(MKL_OneButtonTiny::OCS_COUNT);
      break;

    case MKL_OneButtonTiny::OCS_COUNT:
    ...
    case MKL_OneButtonTiny::OCS_PRESS:
      // waiting for pin being release after long press.

      if (!activeLevel) {
        _newState(MKL_OneButtonTiny::OCS_PRESSEND);
        _startTime = now;
      }  // if
      break;

    case MKL_OneButtonTiny::OCS_PRESSEND:
      // button was released.
      reset();
      break;
    ...
    default:
      ...
      _newState(MKL_OneButtonTiny::OCS_INIT);
      break;
  }  // if
