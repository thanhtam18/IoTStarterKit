 ...
 */
MKL_OneButton::MKL_OneButton(const int pin, const boolean activeLow, const bool pullupActive)
{
  _pin = pin;
...
void MKL_OneButton::setDebounceMs(const unsigned int ms)
{
  _debounce_ms = ms;
} // setDebounceMs
...
void MKL_OneButton::setClickMs(const unsigned int ms)
{
  _click_ms = ms;
} // setClickMs
...
void MKL_OneButton::setPressMs(const unsigned int ms)
{
  _press_ms = ms;
} // setPressMs

// explicitly set the number of millisec that have to pass by before button idle is detected.
void MKL_OneButton::setIdleMs(const unsigned int ms)
{
  _idle_ms = ms;
} // setIdleMs

// save function for click event
void MKL_OneButton::attachClick(callbackFunction newFunction)
{
...
void MKL_OneButton::attachClick(parameterizedCallbackFunction newFunction, void *parameter)
{
...
void MKL_OneButton::attachDoubleClick(callbackFunction newFunction)
{
...
void MKL_OneButton::attachDoubleClick(parameterizedCallbackFunction newFunction, void *parameter)
{
...
void MKL_OneButton::attachMultiClick(callbackFunction newFunction)
{
...
void MKL_OneButton::attachMultiClick(parameterizedCallbackFunction newFunction, void *parameter)
{
...
void MKL_OneButton::attachLongPressStart(callbackFunction newFunction)
{
...
void MKL_OneButton::attachLongPressStart(parameterizedCallbackFunction newFunction, void *parameter)
{
...
void MKL_OneButton::attachLongPressStop(callbackFunction newFunction)
{
...
void MKL_OneButton::attachLongPressStop(parameterizedCallbackFunction newFunction, void *parameter)
{
...
void MKL_OneButton::attachDuringLongPress(callbackFunction newFunction)
{
...
void MKL_OneButton::attachDuringLongPress(parameterizedCallbackFunction newFunction, void *parameter)
{
...
void MKL_OneButton::attachIdle(callbackFunction newFunction)
{
  _idleFunc = newFunction;
} // attachIdle


void MKL_OneButton::reset(void)
{
  _state = MKL_OneButton::OCS_INIT;
  _nClicks = 0;
  _startTime = millis();
  _idleState = false;
}
...
int MKL_OneButton::getNumberClicks(void)
{
  return _nClicks;
}
 ...
 */
int MKL_OneButton::debounce(const int value) {
...
void MKL_OneButton::tick(bool activeLevel)
{
 ...
 */
void MKL_OneButton::_newState(stateMachine_t nextState)
{
  _state = nextState;
} // _newState()
 ...
 */
void MKL_OneButton::_fsm(bool activeLevel)
{
  unsigned long waitTime = (now - _startTime);

  // Implementation of the state machine
  switch (_state) {
  case MKL_OneButton::OCS_INIT:
    ...
    if (activeLevel) {
      _newState(MKL_OneButton::OCS_DOWN);
      _startTime = now; // remember starting time
      _nClicks = 0;
    } // if
    break;

  case MKL_OneButton::OCS_DOWN:
    // waiting for level to become inactive.

    if (!activeLevel) {
      _newState(MKL_OneButton::OCS_UP);
      _startTime = now; // remember starting time

    } else if (waitTime > _press_ms) {
      if (_longPressStartFunc) _longPressStartFunc();
      if (_paramLongPressStartFunc) _paramLongPressStartFunc(_longPressStartFuncParam);
      _newState(MKL_OneButton::OCS_PRESS);
    } // if
    break;

  case MKL_OneButton::OCS_UP:
    // level is inactive

    // count as a short button down
    _nClicks++;
    _newState(MKL_OneButton::OCS_COUNT);
    break;

  case MKL_OneButton::OCS_COUNT:
  ...
  case MKL_OneButton::OCS_PRESS:
    // waiting for pin being release after long press.

    if (!activeLevel) {
      _newState(MKL_OneButton::OCS_PRESSEND);
  ...
  case MKL_OneButton::OCS_PRESSEND:
    // button was released.
  ...
  default:
    ...
    _newState(MKL_OneButton::OCS_INIT);
    break;
  } // if
